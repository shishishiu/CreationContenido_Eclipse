...
public class Solicitar extends HttpServlet {
	...
	/** Nombre de la página **/
	private final String NOMBRE_DE_PAGINA = "Solicitar.jsp";
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
    	...
    	try {
    		...
    		if(usuario.IsAutorizado()){ 
    			if(usuario.getPerUsu() == Common.PERMISO_ASESOR ||
	...
	private void SolicitarMateria(HttpServletRequest request) throws Exception {
	...
	private boolean Validaction(HttpServletRequest request, String cveMat) throws Exception {
		...
		try {
			...
			list = bean.Buscar();
			if(list.size()>0 && 
					list.get(0).getEstadoSolicitud() != Common.ESTADO_DE_SOLICITUD_PENDIENTE){
				
				request.setAttribute(KEY_VARIABLE_MESSAGE, Common.MENSAJE_SOLICITAR_HA_SOLICITADO);
				return false;
	...
	private void InsertarSolicitud(HttpServletRequest request, String cveMat) throws Exception {
		...
		try {
			...
			bean.setEstadoSolicitud(Common.ESTADO_DE_SOLICITUD_SOLICITADO);
			bean.setUsuarioSolicitud(usuario.getCveUsu());
			
			bean.Insertar(con);
			
			Common.InsertLogAct(request, con, usuario.getCveUsu(), 
					MessageFormat.format(Common.TEXTO_ACTION_LOG_SOLICITAR,cveMat));
